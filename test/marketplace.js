...
function assertListing(actual, expected) {
  ...
  assert.equal(actual.price.toString(), expected.price.toString(), "Price is not correct");
...
contract("Marketplace", function (accounts) {
  ...
  before('should reuse variables', async () => {
    ...
    architectureNFT = await ArchitectureNFT.deployed();
    nftContract = architectureNFT.address;
  ...
  it("should list nft", async function () {
    let tokenID = await mintNft(architectureNFT, TOKEN_OWNER);
  ...
  it("should validate before reselling", async function () {
    let tokenID = await mintNft(architectureNFT, TOKEN_OWNER);
    await marketplace.listNft(nftContract, tokenID, ether(".005"), {from: TOKEN_OWNER, value: listingFee});
  ...
  it("should resell nft", async function () {
    ...
    assertListing(getListing(await marketplace.getMyNfts({from: BUYER}), tokenID), expectedListing);
    await architectureNFT.approve(marketplace.address, tokenID, {from: BUYER});
